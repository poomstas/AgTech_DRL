...
def parse_arguments(parser):
    ...
    parser.add_argument('--sigma_init',     type=float, default=0.15,       help='StDev for Ornstein-Uhlenbeck process')
    parser.add_argument('--sigma_decay',    type=float, default=1.0,        help='Decay rate for sigma')
...
def get_writer_name(args):
    writer_name = \
        "DDPG_alpha_{}_beta_{}_tau_{}_sigma_{}_sigmadecay_{}_batchsize_{}_layer1size_{}_layer2size_{}_{}".format(
            args.alpha, args.beta, args.tau, args.sigma_init, args.sigma_decay, args.batch_size, 
...
# %%
def has_plateaued(reward_history, patience=5):
    ...
    plateau_bool = np.abs(single_patience_mean - double_patience_mean) < 1
...
def ddpg_train(args, writer):
    ...
    agent = Agent(alpha=args.alpha, beta=args.beta, input_dims=[11], tau=args.tau, sigma_init=args.sigma_init, sigma_decay=args.sigma_decay,
    ...
    for i in range(30000): # Approx 30,000 iter/day (will almost certainly terminate before)
        ...
        while not done:
            act, sigma = agent.choose_action(obs)
            writer.add_scalar('sigma', sigma, i)
        ...
        if has_plateaued(reward_history):
            print("\nReached Plateau; Increasing Sigma to 0.5.\n")
            agent.reset_OU()
...
def ddpg_load_and_run():
    ...
    agent = Agent(alpha=1, beta=1, input_dims=[11], tau=1, sigma_init=0.15, TB_name="", 
